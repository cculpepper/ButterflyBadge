...
impl LayoutCreator for GridLayoutCreator {
    fn show(&mut self, ui: &mut egui::Ui) -> bool {
        ...
        ui.horizontal(|ui: &mut egui::Ui| {
            ...
            changed |= ui
                .add(egui::Slider::new(&mut self.grid_size[0], 1..=100))
                .changed();

            ui.label("grid_size_y");
            changed |= ui
                .add(egui::Slider::new(&mut self.grid_size[1], 1..=100))
                .changed();
        });
        changed
    }
...
impl RadialLayoutCreator {
    ...
    fn angle_dist_to_vec(start: [f64; 2], angle: f64, dist: f64) -> [f64; 2] {
        let change_vec = [angle.cos(), angle.sin()];
        [
    ...
    fn within_dist_of_other(candidate: [f64; 2], dist: f64, points: &[[f64; 2]]) -> bool {
        points.iter().any(|p| Self::dist(candidate, *p) <= dist)
    }
...
impl LayoutCreator for RadialLayoutCreator {
    fn show(&mut self, ui: &mut egui::Ui) -> bool {
        ...
        ui.horizontal_wrapped(|ui: &mut egui::Ui| {
            ui.label("radius");
            changed |= ui
                .add(egui::Slider::new(&mut self.radius, (0.001)..=0.2))
                .changed();
            ...
            ui.label("cone_size");
            changed |= ui
                .add(egui::Slider::new(&mut self.cone_size, (0.1)..=consts::TAU))
                .changed();

            ui.label("start_x");
            changed |= ui
                .add(egui::Slider::new(&mut self.start[0], (0.)..=1.))
                .changed();

            ui.label("start_y");
            changed |= ui
                .add(egui::Slider::new(&mut self.start[1], (0.)..=1.))
                .changed();
        });
        changed
    }
...
impl Default for MultiLayoutCreator {
    fn default() -> Self {
        ...
        creators.push((String::from("Grid"), Box::new(GridLayoutCreator::default())));
...
fn adjust_uvs_to_fill(uvs: &mut [Vec2]) {
    ...
    let uv_for_new_min_max = |uv: Vec2| -> Vec2 { (uv - min) / (max - min) };
